$(function(){
	...
	$(".midbox-wrapper").on("click",".add-modle-btn",function(){
		...
		toCloneObj.oIndex = $this.closest(".section-item").find(".inline-box").html()
	})
	...
	$(".modal").on("click" , ".btn-primary" ,function(){
		...
		var $sectionNoId = toCloneObj.oIndex; //当前商品编号
		var $Show = $("input[name='add-name-radio1']:checked").val();
		var modalnametxt = $("input[name='nametext']").val();//模型名称
		...
		fullscreenObj.fullscreenHide();
		modalHidefn();
		console.log($sectionNoId)
		console.log(modalnametxt)
		return false;
	});
